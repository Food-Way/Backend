...
class UpvoteServiceTest {
    ...
    @Test
    @DisplayName("Should return all establishment")
    void getAll() {
        Mockito.when(upvoteRepository.findAll()).thenReturn(List.of(new Upvote(), new Upvote()));
        List<Upvote> upvotes = upvoteService.getAll().getBody();

        assertNotNull(upvotes);
        assertEquals(upvotes.size(), 2);
    }
    ...
    void get() {
        ...
        Upvote u = upvoteService.get(id).getBody();
        assertNotNull(u);
    ...
    void post() {
        ...
        Upvote upvote = upvoteService.post(newUpvote).getBody();
    ...
    void put() {
        ...
        Upvote u = upvoteService.put(id, newUpvote).getBody();
        assertNotNull(u);
